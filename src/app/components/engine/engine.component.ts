...
export class EngineComponent {
  ...
  constructor(private fb:FormBuilder,private viewPortScroller:ViewportScroller,private runService:RunService,private customerService:CustomerService){
    ...
    this.val_ini=this.fb.group({
      ...
      val_t:["",[Validators.required,Validators.min(0),Validators.max(99)]],
      tie_p:["",[Validators.required,Validators.min(1),Validators.max(30)]],
      plazo:["Ninguno",[Validators.required]],
      plazo_p_uno:["",[Validators.min(1)]],
      plazo_p_dos:["",[Validators.min(1)]],
      plazo_p_tres:["",[Validators.min(1)]],
      cok:["",[Validators.required,Validators.min(0),Validators.max(99)]]
    }
    )
  }
  pasarDatos(){
